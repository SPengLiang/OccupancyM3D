...
def random_image_flip_horizontal(image, depth_map, gt_boxes, calib, gt_boxes2d, lidar_depth):
    """
    ...
    if enable:
        ...
        x1 = aug_gt_boxes2d[:, 0]
        x2 = aug_gt_boxes2d[:, 2]
        aug_gt_boxes2d[:, 0], aug_gt_boxes2d[:, 2] = W - x2, W - x1

        # print(gt_boxes2d, aug_gt_boxes2d, W)

        '''flip lidar depth map'''
        lidar_depth = np.fliplr(lidar_depth)
    ...
    from . import data_aug_affine
    aug_lidar_3d, _ = data_aug_affine.convert_to_3d(lidar_depth, calib.P2, 1, 0, 0)
    aug_lidar_3d = calib.rect_to_lidar(aug_lidar_3d)
    aug_lidar_coor_map = np.zeros((image.shape[0], image.shape[1], 3))
    aug_lidar_coor_map[..., 0] = aug_lidar_3d[:, 0].reshape(image.shape[0], image.shape[1])
    aug_lidar_coor_map[..., 1] = aug_lidar_3d[:, 1].reshape(image.shape[0], image.shape[1])
    aug_lidar_coor_map[..., 2] = aug_lidar_3d[:, 2].reshape(image.shape[0], image.shape[1])

    return aug_image, aug_depth_map, aug_gt_boxes, aug_gt_boxes2d, lidar_depth, aug_lidar_coor_map
...
def get_points_in_box(points, gt_box, MARGIN=1e-1):
